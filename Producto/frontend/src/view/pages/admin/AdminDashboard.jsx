...
import { useAdminDashboardViewModel } from '@/viewmodel/admin/useAdminDashboardViewModel'
...
function StatCard({ icon, label, value, to, color = 'text-secondary' }) {
  return (
    ...
    >
      <div className="flex size-12 flex-shrink-0 items-center justify-center rounded-xl bg-secondary/10">
...
// Mapa estático de clases por estado: no depende de props ni de estado local,
// así que vive a nivel de módulo en vez de reconstruirse en cada render.
const ESTADO_CLS = {
...
export function EstadoBadge({ estado }) {
  return (
    <span className={`flex-shrink-0 rounded px-2 py-0.5 text-[11px] font-semibold ${ESTADO_CLS[estado] ?? ESTADO_CLS.oculta}`}>
      {estado}
    </span>
  )
}
