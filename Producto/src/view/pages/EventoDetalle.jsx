...
import { useEventoDetalleViewModel } from '@/viewmodel/public/useEventoDetalleViewModel'
import { formatPrecio, formatFecha } from '@/core/utils'

export function EventoDetallePage() {
  const vm = useEventoDetalleViewModel()

  if (vm.loading) return <DetalleSkeleton />
  if (vm.error || !vm.evento) return <NoEncontrado />

  const { evento, lugar, punto, fallback } = vm
