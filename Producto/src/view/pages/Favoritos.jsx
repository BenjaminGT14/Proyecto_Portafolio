...
import { useFavoritosViewModel } from '@/viewmodel/public/useFavoritosViewModel'

export function FavoritosPage() {
  const vm = useFavoritosViewModel()
          ...
          {vm.lugares.length > 0 && (
            <section className="space-y-4">
              <h2 className="flex items-center gap-2 text-xl font-semibold tracking-tight">
                <Icon name="place" />
                Lugares
                <span className="text-base font-normal text-outline">({vm.lugares.length})</span>
              </h2>
          ...
          {vm.eventos.length > 0 && (
            <section className="space-y-4">
              <h2 className="flex items-center gap-2 text-xl font-semibold tracking-tight">
                <Icon name="event" />
                Eventos
                <span className="text-base font-normal text-outline">({vm.eventos.length})</span>
              </h2>
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {vm.eventos.map((e) => (
